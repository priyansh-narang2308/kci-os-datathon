...
export default function InvestigationPage() {
  ...
  const [ingestForm, setIngestForm] = useState<FIRIngestPayload>({
    crime_type: "theft",
  ...
  const [ingestResult, setIngestResult] = useState<FIRIngestResponse | null>(
    null,
  );
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "bot",
          content:
            "I encountered an error processing your query. Please try again or rephrase.",
          timestamp: botTime,
          richData: null,
        },
      ]);
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "system",
          content: `**System Alert: New FIR Ingested** — ${result.fir_no} (${ingestForm.crime_type} in ${ingestForm.district})`,
          timestamp: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
      ...
      setStats((prev) =>
        prev
          ? {
              ...
              new_firs_this_week: prev.new_firs_this_week + 1,
            }
          : prev,
      );
      setActivity((prev) => [
        {
          action: `FIR Ingested: ${result.fir_no}`,
          detail: `${ingestForm.crime_type} in ${ingestForm.district} — ${result.steps.filter((s) => s.status === "complete").length} cascade stages`,
          time: "Now",
          badge: "SYSTEM",
        },
        ...prev,
      ...
      setIngestResult({
        ...
        fir_no: "",
        fir: null,
        steps: [{ step: "error", status: "failed", detail: e.message }],
        alert: null,
        early_warning: null,
        total_firs: 0,
      });
      ...
      <div className="flex h-full flex-col rounded-2xl border border-border bg-stone-900 p-4 text-white relative overflow-hidden shadow-inner min-h-[350px]">
        ...
        <div className="flex-1 flex items-center justify-center relative my-4">
          ...
          <div className="relative flex flex-col items-center gap-6 z-10 w-full max-w-xs">
            ...
            {edges.length > 0 && (
              <div
                className={`h-6 w-0.5 bg-linear-to-b from-emerald-500 to-amber-500 relative ${isFactsMode ? "opacity-30" : ""}`}
              >
      ...
      return (
        <div className="rounded-xl border border-border bg-muted/50 p-3 text-center">
          <p className="text-[11px] text-muted-foreground">
            No {factMode === "facts_only" ? "verified" : "AI-inferred"} records
            match this filter.
          </p>
        </div>
      );
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-4 shadow-2xs space-y-3">
        <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
          <Filter className="size-3" /> Retrieved Entity Records (
          {filteredCitations.length})
          {selectedQueryData?.fact_mode === "hybrid" &&
            factMode === "hybrid" && (
              <span className="ml-auto text-[10px] font-normal text-emerald-600">
                {citations.filter((c) => c.is_verified).length} verified /{" "}
                {citations.filter((c) => !c.is_verified).length} inferred
              </span>
            )}
        </h4>
          ...
          {filteredCitations.map((cit, idx) => (
            <div key={idx} className="p-3 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p
                    className={`font-semibold ${cit.is_verified ? "text-emerald-800" : "text-foreground"}`}
                  >
                    {cit.fir_no}
                    {cit.is_verified && (
                      <span className="ml-1.5 text-[9px] uppercase text-emerald-600 font-bold">
                        Verified
                      </span>
                    )}
                  </p>
              ...
              {cit.score !== undefined && (
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${cit.is_verified ? "bg-emerald-100 text-emerald-700" : cit.score > 0.8 ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}
                >
          ...
          <div className="rounded-xl border border-border bg-card p-3 shadow-2xs">
            ...
            <div className="flex items-center gap-4 text-[11px]">
              ...
              <span className="flex items-center gap-1.5 text-amber-700">
                <div className="size-3.5 rounded-full border border-dashed border-amber-400" />{" "}
                AI-Inferred
              </span>
            </div>
          </div>
            ...
            return (
              <div className="rounded-2xl border border-border bg-card p-4 shadow-2xs space-y-2">
                <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                  <Zap className="size-3.5 text-emerald-500" /> Ingestion
                  Cascade Pipeline
                </h4>
                <div className="relative pl-5 space-y-2 before:absolute before:left-2 before:top-1.5 before:bottom-1 before:w-0.5 before:bg-muted">
                  {result.steps.map((step, idx) => (
                    <div key={idx} className="relative flex items-start gap-2">
                      <span
                        className={`absolute left-[-17px] top-1 flex size-3 items-center justify-center rounded-full ring-2 ring-card ${step.status === "complete" ? "bg-emerald-500" : step.status === "alert" ? "bg-amber-500" : step.status === "failed" ? "bg-red-500" : "bg-muted-foreground"}`}
                      />
                ...
                {result.early_warning && (
                  <div
                    className={`mt-2 rounded-lg p-2 text-[11px] font-medium flex items-center gap-2 ${result.early_warning.level === "critical" ? "bg-red-50 text-red-700 border border-red-200" : "bg-amber-50 text-amber-700 border border-amber-200"}`}
                  >
                    <AlertTriangle className="size-3.5 shrink-0" />
                    {result.early_warning.level.toUpperCase()} Early Warning for{" "}
                    {result.early_warning.crime_type} in{" "}
                    {result.early_warning.district}:{" "}
                    {result.early_warning.next_7_total} predicted next 7 days
                  </div>
                )}
              </div>
            );
  ...
  const renderPanelContent = () => (
    ...
    >
      <div className="border-b border-border bg-muted/30 px-3 py-2.5">
        <TabsList className="flex w-full items-center justify-start gap-1 overflow-x-auto no-scrollbar bg-card/90 p-1.5 shadow-xs rounded-xl border border-border">
          {[
            ...
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="flex shrink-0 items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-muted-foreground transition-all cursor-pointer data-[state=active]:bg-emerald-600 data-[state=active]:text-white data-[state=active]:shadow-xs hover:text-foreground"
            >
        ...
        <div className="space-y-4">
          <div className="rounded-xl bg-muted p-3 text-xs font-medium border border-border">
            <p className="flex items-center gap-1.5 text-foreground mb-2">
              <Scale className="size-3.5 text-emerald-600" /> Evidence
              Classification Matrix
            </p>
            <p className="text-[11px] text-muted-foreground">
              Toggle between court-verified facts (green) and AI-inferred
              hypotheses (dashed) to understand what is proven vs suggested.
            </p>
          </div>
          {renderFactModeToggle()}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-emerald-700/70">
                    3 FIRs verified by CMM/JMFC courts
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-semibold text-amber-800">
                    AI-Inferred Hypotheses
                  </p>
                  <p className="text-[10px] text-amber-700/70">
                    Based on MO similarity, network proximity, temporal patterns
                  </p>
                </div>
              </div>
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col xl:flex-row bg-background overflow-hidden">
        ...
        <DialogContent className="sm:max-w-[425px] bg-background">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Simulate a live FIR ingestion. The system will run Crime DNA,
              network analysis, and early warning evaluation.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Select
                value={ingestForm.crime_type}
                onValueChange={(v) =>
                  setIngestForm((p) => ({ ...p, crime_type: v }))
                }
              >
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={ingestForm.district}
                onValueChange={(v) =>
                  setIngestForm((p) => ({ ...p, district: v }))
                }
              >
                <SelectTrigger className="h-9 text-xs">
                  <SelectValue />
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setIngestForm((p) => ({ ...p, description: e.target.value }))
                }
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setIngestForm((p) => ({ ...p, suspect_name: e.target.value }))
                }
            ...
            >
              ...
              {ingesting
                ...
                : "Ingest FIR & Run Cascade"}
            </Button>

            {ingestResult && (
              <div className="rounded-xl border border-border bg-card p-3 space-y-1.5">
                <p
                  className={`text-xs font-bold flex items-center gap-1.5 ${ingestResult.success ? "text-emerald-700" : "text-red-700"}`}
                >
                ...
                {ingestResult.steps.map((s, i) => (
                  ...
                  >
                    <span
                      className={`size-1.5 rounded-full ${s.status === "complete" ? "bg-emerald-500" : s.status === "alert" ? "bg-amber-500" : "bg-red-500"}`}
                    />
                    {s.step}: {s.detail}
                  </p>
                ))}
      ...
      <div className="flex min-w-0 min-h-0 flex-1 flex-col bg-background">
        <div className="flex xl:hidden items-center justify-between border-b border-border bg-card px-4 py-2.5 shadow-2xs">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-xs font-semibold text-emerald-700 shadow-2xs active:scale-[0.98]"
            >
        ...
        <div className="hidden xl:flex items-center justify-between border-b border-border bg-card px-4 py-2">
          <div className="flex items-center gap-2">
            <span
              className={`flex size-2 rounded-full ${isReadOnly ? "bg-amber-500" : "bg-emerald-500"} animate-pulse`}
            />
            <span className="text-xs font-bold text-foreground/80">
              {isReadOnly
                ? "Investigation Copilot (Read-Only)"
                : "Investigation Copilot"}
            </span>
            {isReadOnly && (
              <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[9px] font-bold text-amber-700 uppercase border border-amber-200">
                Review Mode
              </span>
            )}
          </div>
        ...
        {isReadOnly && (
          <div className="bg-amber-50 border-b border-amber-200 px-4 py-2 text-[11px] font-medium text-amber-800 flex items-center gap-2">
            <Eye className="size-3.5" /> You are in{" "}
            <strong>read-only review mode</strong>. Investigation queries are
            viewable but new FIR ingestion is disabled for your role.
          </div>
        )}
                          ...
                          {msg.citations.map((cit, ci) => (
                            ...
                            >
                              {cit.is_verified ? (
                                ...
                                <div className="size-2 rounded-full border border-dashed border-amber-400" />
                              )}
                              {cit.fir_no}
                            </span>
                          ))}
