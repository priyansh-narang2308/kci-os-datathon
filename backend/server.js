...
const USERS = {
  ...
  investigator: {
    password: hashPassword("invest123"),
    role: "investigator",
    name: "Inspector Sharma",
    badge: "Inspector Rank",
  },
  analyst: {
  ...
  supervisor: {
    ...
    role: "supervisor",
    name: "DG Iyer",
    badge: "Deputy General",
  },
  policymaker: {
    password: hashPassword("policy123"),
    role: "policymaker",
    name: "Secretary Nair",
    badge: "Policy Advisor",
  },
};
...
const edgesData = fs.existsSync(EDGES_PATH)
  ? JSON.parse(fs.readFileSync(EDGES_PATH, "utf-8"))
  : null;
const alerts = [];

// ── Load Catalyst Function Handlers (with SDK mock for local dev) ──
const {
  handlers: catalystHandlers,
  setStoreData,
} = require("./catalyst/loader");
...
} = require("./engines/investigation-support.js");
  ...
  async query(cypher, params = {}) {
    ...
    if (upper.includes("involved_in")) {
      ...
      for (const fir of firs) {
        if (fir.accused_ids?.includes(id)) {
          firCount++;
          crimeTypes.add(fir.crime_type);
        }
      }
      ...
      while (queue.length > 0) {
        ...
        if (distance > 0) {
          results.push({
            ...
            type: "Accused",
            distance,
            edge_types: edgeTypes.length > 0 ? edgeTypes : ["linked_to"],
          });
        }
...
async function ensureCrimeDNA() {
  if (!crimeDNAInitialized) {
    await crimeDNA.initialize(firs);
    crimeDNAInitialized = true;
  }
}

async function ensureForecasting() {
  if (!forecastingInitialized) {
    await forecasting.initialize(firs);
    forecastingInitialized = true;
  }
}

async function ensureInvestigation() {
  if (!investigationInitialized) {
    await investigation.initialize(firs);
    investigationInitialized = true;
  }
}

app.post(
  "/api/nlu",
  ...protect("investigator", "analyst", "admin"),
  (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      const rp = [
        {
          ...
          desc: `Parsed query: intent='${intent}', slots=${JSON.stringify(slots)}`,
          conf: "99.8%",
        },
      ];
      ...
      if (intent === "show_network" || intent === "network_analysis") {
        ...
        let netResult;
        try {
          netResult = await networkAnalyzer.getNetworkAround(target, 2);
        } catch {
          netResult = { nodes: [], edges: [] };
        }
        const viz = netResult.nodes
          ? networkAnalyzer.formatForVisualization(netResult)
          : { nodes: [], edges: [] };
        rp.push({
          title: "Graph Traversal",
          desc: `BFS from '${target}' found ${viz.nodes.length} nodes`,
          conf: "100%",
        });
        rp.push({
      ...
      if (intent === "find_similar" || intent === "investigation_support") {
        ...
        const tf = slots.fir_number
          ? firs.find((f) => f.fir_no === slots.fir_number)
          : firs[0];
        if (tf) {
          const sim = await investigation.findSimilar(tf, { topK: 5 });
          rp.push({
            title: "Embedding Search",
            desc: `Found ${sim.count} similar cases`,
            conf: "94.2%",
          });
          rp.push({
            title: "Spatio-Temporal Synthesis",
            desc: "Aggregated results and formulated investigative recommendations.",
            conf: "96.5%",
          });
          return res.json({
            type: "similar",
            response: sim.report,
            citations: sim.similar_cases.map((c) => ({
              fir_no: c.fir_no,
              crime_type: c.crime_type,
              district: c.district,
              score: c.score,
              is_verified: FACT_STORE.has(c.fir_no),
            })),
            confidence: sim.similar_cases[0]?.score || 0,
            reasoning_path: rp,
            intent,
            slots,
            language,
        ...
        return res.json({
          type: "response",
          response: `FIR "${slots.fir_number}" not found.`,
          citations: [],
          confidence: 0,
          reasoning_path: rp,
          intent,
      ...
      if (
        ...
        let fd;
        try {
          fd = await forecasting.getForecast(ct, dist, 30);
        } catch {
          fd = null;
        }
        rp.push({
          title: "Temporal Forecasting",
          desc: `${ct} in ${dist}: next 7d=${fd?.next_7_days_total || "N/A"}`,
          conf: "88.3%",
        });
        rp.push({
          title: "Spatio-Temporal Synthesis",
        ...
        return res.json({
          type: "forecast",
          response: fd
            ? `**Forecast for ${ct} in ${dist}**: Next 7 days: **${fd.next_7_days_total}** incidents, Next 30 days: **${fd.next_30_days_total}** incidents. Model accuracy: MAPE=${(fd.model_stats.mape * 100).toFixed(1)}%.`
            : `No forecast model available for ${ct} in ${dist}.`,
          citations: [],
          confidence: fd ? 0.88 : 0,
          reasoning_path: rp,
          forecast: fd,
          hotspots: forecasting.getHotspots(dist, 5).slice(0, 5),
          intent,
          slots,
          language,
          fact_mode: "hybrid",
      ...
      if (intent === "crime_dna") {
        ...
        if (tf) {
          ...
          rp.push({
            title: "MO Feature Extraction",
            desc: `Extracted ${Object.keys(an.features).length} MO features`,
            conf: "99.8%",
          });
          rp.push({
            title: "Vector Proximity Search",
            desc: `Retrieved ${an.match_count} similar embeddings`,
            conf: `${((an.matches[0]?.score || 0) * 100).toFixed(1)}%`,
          });
          rp.push({
            title: "Pattern Detection",
            desc: an.pattern_detected
              ? "MO pattern cluster detected"
              : "No significant pattern",
            conf: an.pattern_detected ? "87.2%" : "95.1%",
          });
          rp.push({
            title: "Spatio-Temporal Synthesis",
            desc: "Aggregated results and formulated institutional intelligence advice.",
          ...
          return res.json({
            type: "crime_dna",
            response: an.report,
            citations: an.matches.map((m) => ({
              fir_no: m.fir_no,
              crime_type: m.crime_type,
              district: m.district,
              score: m.score,
              is_verified: FACT_STORE.has(m.fir_no),
            })),
            confidence: an.matches[0]?.score || 0,
            reasoning_path: rp,
            intent,
            slots,
            language,
      ...
      if (lq.includes("chain-snatching") || lq.includes("mysuru")) {
        const ms = firs
          .filter(
            (f) =>
              f.district === "Mysuru" && f.crime_type === "chain_snatching",
          )
          .slice(0, 3);
        ...
        lq.includes("repeat") ||
        lq.includes("offender") ||
        lq.includes("bengaluru")
      ) {
        const bfs = firs.filter((f) => f.district === "Bengaluru Urban");
        const ac = {};
        for (const f of bfs) {
          if (f.accused_ids)
            f.accused_ids.forEach((a) => {
              ac[a] = (ac[a] || 0) + 1;
            });
        }
      ...
      res.json({
        ...
        citations: citations
          .slice(0, 5)
          .map((c) => ({ ...c, is_verified: FACT_STORE.has(c.fir_no) })),
        confidence,
        reasoning_path: rp,
        intent,
        slots,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const target =
        (req.body.fir_no
          ? firs.find((f) => f.fir_no === req.body.fir_no)
          : null) || firs[0];
      const result = await crimeDNA.analyzeNewFIR(target);
      res.json({
        fir_no: target.fir_no,
        mo_features: result.features,
        matches: result.matches,
        match_count: result.match_count,
        pattern_detected: result.pattern?.pattern_detected || false,
...
app.post(
  "/api/forecast",
  ...protect("analyst", "supervisor", "policymaker", "admin"),
  async (req, res) => {
    try {
      await ensureForecasting();
      const ct = req.body.crime_type || "theft",
        dist = req.body.district || "Bengaluru Urban",
        d = req.body.days || 30;
...
app.get(
  "/api/stats",
  ...protect("investigator", "analyst", "supervisor", "policymaker", "admin"),
  (req, res) => {
    try {
      const districts = [...new Set(firs.map((f) => f.district))].length;
      const oneWeekAgo = new Date();
      oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
      const newThisWeek = firs.filter(
        (f) => new Date(f.date_filed) >= oneWeekAgo,
      ).length;
      res.json({
        ...
        engines_online: 5,
        districts,
        new_firs_this_week: newThisWeek,
        alerts_generated: 3,
      });
    } catch (err) {
      res.status(500).json({ error: err.message });
    ...
    res.json([
      {
        ...
        detail: `87% MO similarity — FIR ${firs[r()].fir_no}`,
        time: "2 min ago",
        badge: "Match",
      },
      {
...
app.get("/api/heatmap", ...protect("analyst", "admin"), async (req, res) => {
  try {
    await ensureForecasting();
    res.json(forecasting.getHeatmapData());
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get(
  "/api/alerts",
  ...protect("investigator", "analyst", "supervisor", "admin"),
  async (req, res) => {
    try {
      await ensureForecasting();
      const earlyWarnings = forecasting.evaluateEarlyWarnings();
      const forecastAlerts = (
        Array.isArray(earlyWarnings) ? earlyWarnings : []
      ).map((ew, i) => ({
        id: `fw_${i}_${Date.now()}`,
        fir_no: "SYSTEM",
        type: "forecast",
          ...
          try {
            ...
            if (f)
              summaries.push({
                ...
                crime_type: ct,
                risk:
                  f.next_7_days_total > 10
                    ? "critical"
                    : f.next_7_days_total > 5
                      ? "elevated"
                      : "normal",
                next_7: f.next_7_days_total,
              });
          } catch {}
        }
      }
...
FACT_STORE.set("ACC_001", {
  verified: true,
  verified_by: "CMM Court Bengaluru",
  verified_at: "2026-01-15",
});
FACT_STORE.set("FIR_2025_011", {
  verified: true,
  verified_by: "JMFC Court Mysuru",
  verified_at: "2026-02-20",
});
FACT_STORE.set("FIR_2025_024", {
  verified: true,
  verified_by: "CMM Court Belagavi",
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!crime_type || !district)
        return res
          .status(400)
          .json({ error: "Missing required fields: crime_type, district" });

      const newFirNo = `FIR_2026_${String(firs.length + 1).padStart(4, "0")}`;
      const loc = location || {
        lat: 12.97 + Math.random() * 0.06,
        lng: 77.57 + Math.random() * 0.06,
      };
      const newFIR = {
        fir_no: newFirNo,
        crime_type,
        ...
        narrative_text:
          description || `${crime_type} incident reported in ${district}`,
        description:
          description || `${crime_type} incident reported in ${district}`,
        ...
        accused_ids:
          suspect_name !== "Unknown"
            ? [`ACC_${String(firs.length + 1).padStart(4, "0")}`]
            : [],
      ...
      try {
        const entityResult = await catalystHandlers[
          "entity-extraction"
        ].handler({ data: newFIR });
        if (entityResult.status === 200) {
          steps.push({
            step: "catalyst_entity_extraction",
            status: "complete",
            detail: `Extracted ${entityResult.content.entities.accused.length} accused, ${entityResult.content.entities.victims.length} victims — Catalyst Function`,
          });
        } else {
          steps.push({
            step: "catalyst_entity_extraction",
            status: "failed",
            detail: entityResult.content.error,
          });
        }
      } catch (e) {
        steps.push({
      ...
      let dnaResult = null;
      try {
        const dnaHandlerResult = await catalystHandlers[
          "crime-dna-analyzer"
        ].handler({ data: newFIR });
        if (dnaHandlerResult.status === 200) {
          dnaResult = dnaHandlerResult.content;
          steps.push({
            step: "catalyst_crime_dna",
            status: "complete",
            detail: `Matched ${dnaResult.match_count} similar cases (score: ${dnaResult.matches?.[0]?.score || "N/A"}) — Catalyst Function`,
          });
        } else {
          ...
          steps.push({
            step: "catalyst_crime_dna",
            status: "complete",
            detail: `Matched ${dnaResult.match_count} similar cases (local fallback)`,
            fallback: true,
          });
        }
      } catch (e) {
      ...
      try {
        networkImpact = await networkAnalyzer.getNetworkAround(
          newFIR.fir_no,
          1,
        );
        steps.push({
          step: "network",
          status: "complete",
          detail: `Network expanded — ${networkImpact.nodes?.length || 0} nodes reachable from ${newFIR.fir_no}`,
        });
      } catch (e) {
      ...
      let simCases = null;
      try {
        const simResult = await catalystHandlers["similar-cases"].handler({
          data: { fir_no: newFIR.fir_no },
        });
        if (simResult.status === 200) {
          simCases = simResult.content;
          steps.push({
            step: "catalyst_similar_cases",
            status: "complete",
            detail: `Found ${simCases.count} similar historical cases — Catalyst Function`,
          });
        } else {
          ...
          steps.push({
            step: "catalyst_similar_cases",
            status: "complete",
            detail: `Found ${simCases.count} similar cases (local fallback)`,
          });
        }
      } catch (e) {
        simCases = await investigation.findSimilar(newFIR, { topK: 3 });
      ...
      try {
        const alertResult = await catalystHandlers["alert-generator"].handler({
          data: { fir_data: newFIR, dna_analysis: dnaResult || {} },
        });
        if (alertResult.status === 200) {
          steps.push({
            step: "catalyst_alert_generator",
            status: "complete",
            detail: `Generated ${alertResult.content.alerts_generated} alerts — Catalyst Function`,
          });
        } else {
        ...
        steps.push({
          step: "catalyst_alert_generator",
          status: "fallback",
          detail: `Local fallback: ${e.message}`,
        });
      }
      ...
      try {
        ...
        if (forecast && forecast.next_7_days_total > 5) {
          earlyWarning = {
            crime_type,
            district,
            level: forecast.next_7_days_total > 10 ? "critical" : "elevated",
            next_7_total: forecast.next_7_days_total,
          };
          steps.push({
            step: "early_warning",
            status: "alert",
            detail: `Forecast predicts ${forecast.next_7_days_total} ${crime_type} cases in ${district} next 7 days — ${earlyWarning.level} risk`,
          });
        } else {
          steps.push({
            step: "early_warning",
            status: "normal",
            detail: `No elevated risk detected for ${crime_type} in ${district}`,
          });
        }
      } catch (e) {
        steps.push({
  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (msg.citations && msg.citations.length > 0) {
        lines.push(
          `  └─ Citations: ${msg.citations.map((c) => c.fir_no).join(", ")}`,
        );
      }
      lines.push("");
    }
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="kcios-export-${Date.now()}.pdf"`,
    );
    res.send(buffer);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post(
  "/api/catalyst/invoke/:functionName",
  ...protect("admin"),
  async (req, res) => {
    try {
      ...
      if (!catalystHandlers[functionName]) {
        return res
          .status(404)
          .json({
            error: `Catalyst function '${functionName}' not found. Available: ${Object.keys(catalystHandlers).join(", ")}`,
          });
      }
      const result = await catalystHandlers[functionName].handler({
        data: req.body,
      });
      res.json({
...
app.get(
  ...
  ...protect("admin", "investigator", "analyst"),
  (req, res) => {
    const fnDescriptions = {
      "entity-extraction":
        "Extract entities and MO features from FIR narrative",
      "crime-dna-analyzer":
        "Run MO similarity search and pattern detection on FIR",
      "alert-generator": "Evaluate early warning rules and generate alerts",
      "nlu-pipeline":
        "Process natural language queries (English, Kannada, Code-Mixed)",
    ...
    const fnList = Object.keys(catalystHandlers).map((name) => ({
      name,
      description: fnDescriptions[name] || "",
      status: "loaded",
    }));
    res.json({
      functions: fnList,
      runtime: "Catalyst Functions (Mock SDK — Local Dev)",
      event_listeners:
        "fir_insert → entity-extraction → crime-dna-analyzer → alert-generator",
    });
  },
);

// ── GET /api/catalyst/capabilities — Audit & return all 26 Zoho Catalyst capabilities ──
const { catalyst26Engine } = require("./catalyst/services-engine");
app.get(
  "/api/catalyst/capabilities",
  ...protect("admin", "investigator", "analyst", "supervisor", "policymaker"),
  async (req, res) => {
    try {
      const auditReport = await catalyst26Engine.verifyAll26Capabilities();
...
// ── GET /api/schema — Return full ER diagram schema documentation ──
app.get("/api/schema", authenticateToken, (req, res) => {
  res.json({
    diagram: "Police FIR System — ER Diagram",
    source: "Karnataka Police Department — Database Design Document",
    total_tables: 26,
    tables: {
      lookup: [
        {
          ...
          columns: 5,
          rows: 1,
          description: "State master (Karnataka)",
        },
        {
          name: "District",
          columns: 4,
          rows: 9,
          description: "District master",
        },
        {
          name: "UnitType",
          columns: 5,
        ...
        },
        {
          ...
          columns: 8,
          rows: 14,
          description: "Police station / unit master",
        },
        {
          name: "Rank",
          columns: 4,
          rows: 13,
          description: "Police rank master",
        },
        {
          name: "Designation",
          columns: 4,
          rows: 7,
          description: "Police designation master",
        },
        {
          name: "Employee",
          columns: 12,
          rows: 5,
          description: "Police employee master",
        },
        {
          name: "CaseCategory",
          columns: 2,
          rows: 4,
          description: "FIR / UDR / Zero FIR / PAR",
        },
        {
          name: "GravityOffence",
          columns: 2,
          rows: 2,
          description: "Heinous / Non-Heinous",
        },
        {
          name: "CrimeHead",
          columns: 3,
          rows: 9,
          description: "Major crime head (e.g. Crimes Against Property)",
        },
        {
          name: "CrimeSubHead",
          columns: 4,
          rows: 13,
          description: "Minor crime sub-head (e.g. Theft, Burglary)",
        },
        {
          name: "Act",
          columns: 4,
          rows: 4,
          description: "Legal act (IPC, NDPS, IT, CrPC)",
        },
        {
          name: "Section",
          columns: 4,
          rows: 70,
          description: "Legal sections under each act",
        },
        {
          name: "CrimeHeadActSection",
          columns: 3,
          rows: 35,
          description: "Maps CrimeHead → Act + Section",
        },
        {
        ...
        { name: "Court", columns: 5, rows: 6, description: "Court master" },
        {
          name: "CasteMaster",
          columns: 2,
          rows: 5,
          description: "Caste lookup",
        },
        {
          name: "ReligionMaster",
          columns: 2,
      ...
      data: [
        {
          ...
          columns: 18,
          rows: 500,
          description: "Main FIR/case table (our primary data)",
        },
        {
          name: "ComplainantDetails",
          columns: 8,
          rows: 500,
          description: "Complainant per FIR",
        },
        {
          name: "Victim",
          columns: 6,
          rows: 621,
          description: "Victim per FIR (from edges)",
        },
        {
          name: "Accused",
          columns: 6,
          rows: 706,
          description: "Accused per FIR (from edges)",
        },
        {
          name: "ActSectionAssociation",
          columns: 6,
          rows: 1000,
          description: "Act-sections per FIR",
        },
        {
          name: "ArrestSurrender",
          columns: 12,
          rows: 0,
          description: "Arrest/surrender tracking",
        },
        {
          name: "ChargesheetDetails",
          columns: 5,
          rows: 200,
          description: "Chargesheet/final report",
        },
      ],
    },
    relationships: [
      ...
      {
        parent: "CaseMaster",
        child: "ComplainantDetails",
        type: "1:N",
        via: "CaseMasterID",
      },
      {
        parent: "CaseMaster",
        child: "ActSectionAssociation",
        type: "1:N",
        via: "CaseMasterID",
      },
      {
    ...
    data_store: "Catalyst Data Store (all tables created via migration script)",
    migration_script: "backend/sql/catalyst-migration.js",
    ddl_schema: "backend/sql/schema.sql",
    seed_data: "backend/sql/seed-reference.sql",
  });
});
